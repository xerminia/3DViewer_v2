...
EGifPutCodeNext(GifFileType *GifFile, const GifByteType *CodeBlock)
{
    ...
    if (CodeBlock != NULL) {
        if ((unsigned int)InternalWrite(GifFile, CodeBlock, CodeBlock[0] + 1)
                   ...
                   int c)
{
    if (c == FLUSH_OUTPUT) {
        ...
        if (Buf[0] != 0
            && (unsigned int)InternalWrite(GifFile, Buf, Buf[0] + 1) != (unsigned)(Buf[0] + 1)) {
            GifFile->Error = E_GIF_ERR_WRITE_FAILED;
            return GIF_ERROR;
        }
        ...
        if (Buf[0] == 255) {
            ...
            if ((unsigned int)InternalWrite(GifFile, Buf, Buf[0] + 1) != (unsigned)(Buf[0] + 1)) {
                GifFile->Error = E_GIF_ERR_WRITE_FAILED;
                return GIF_ERROR;
            }
            Buf[0] = 0;
